    ...
    async execute(interaction, client) {
        ...
        if (interaction.isModalSubmit() && interaction.customId === 'yetkili_form') {
            ...
            await interaction.reply({
                content: '✅ Başvurunuz alınmıştır, teşekkürler!',
                ephemeral: true
            });
        ...
        if (interaction.isButton() && interaction.customId === 'create_ticket') {
            try {
                ...
                const ticketChannel = await guild.channels.create({
                    ...
                    parent: TICKET_CATEGORY_ID, // ✅ düzeltilmiş parametre
        ...
        // 📌 ACTIVE DEVELOPER ROLÜ
        if (interaction.isButton() && interaction.customId === 'get_active_dev_role') {
            try {
                const member = await interaction.guild.members.fetch(interaction.user.id);

                if (!ACTIVE_DEV_ROLE_ID) {
                    return await interaction.reply({
                        content: '⚠️ Rol ID .env dosyasında ayarlanmamış!',
                        ephemeral: true
                    });
                }

                if (member.roles.cache.has(ACTIVE_DEV_ROLE_ID)) {
                    await member.roles.remove(ACTIVE_DEV_ROLE_ID);
                    return await interaction.reply({
                        content: `❌ Active Developer rolü üzerinizden kaldırıldı.`,
                        ephemeral: true
                    });
                } else {
                    await member.roles.add(ACTIVE_DEV_ROLE_ID);
                    return await interaction.reply({
                        content: `✅ Active Developer rolü başarıyla verildi.`,
                        ephemeral: true
                    });
                }

            } catch (err) {
                console.error('Active Developer rolünde hata:', err);
                return await interaction.reply({
                    content: '❌ Active Developer rolü verilirken hata oluştu.',
                    ephemeral: true
                });
            }
            return;
